...
test("Excel 排假表保留紅色休假、週末、三列人力摘要及門店空白列", () => {
  ...
  assert.match(xml, /<Row\/><Row ss:Height="25"><Cell ss:StyleID="StoreTitle"/);
  assert.match(xml, /<Column ss:Width="116\.25"\/>/);
  assert.equal((xml.match(/<Column ss:Width="30\.75"\/>/g) || []).length, 31);
  assert.match(xml, /<Row ss:Height="25"><Cell ss:StyleID="StoreTitle"/);
